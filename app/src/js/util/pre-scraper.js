...
if (yn(process.env.REACT_APP_LOCAL)) {
  ...
  data = fs.readFileSync(process.env.REACT_APP_WEAPONS_URL_LOCAL);
} else {
  data = await axios.get(process.env.REACT_APP_WEAPONS_URL).data;
}
...
export function preScrapeWeapons() {
  ...
  if (yn(process.env.REACT_APP_PRE_SCRAPER_ALLOW_PRIMARY)) {
    scrapePrimaryWeapons(tables[0]);
  }
  if (yn(process.env.REACT_APP_PRE_SCRAPER_ALLOW_SECONDARY)) {
    scrapeSecondaryWeapons(tables[1]);
  }
  if (yn(process.env.REACT_APP_PRE_SCRAPER_ALLOW_MELEE)) {
    scrapeMeleeWeapons(tables[2]);
  }
  if (yn(process.env.REACT_APP_PRE_SCRAPER_ALLOW_ARCHWING)) {
    scrapeArchwingWeapons(tables[3]);
  }
  if (yn(process.env.REACT_APP_PRE_SCRAPER_ALLOW_ROBOTIC)) {
