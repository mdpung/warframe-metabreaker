...
async function enumerateWeapons(type) {
  ...
  if (yn(process.env.LOCAL)) {
    ...
    weapons.forEach(element => {
      enumerateFields(fs.readFileSync(process.env.ENUMERATOR_BASE_PATH + type + "/" + element.name + ".html"), dict);
    });
  } else {
    weapons.forEach(element => {
...
export function enumerateAllWeaponFields() { 
  if (yn(process.env.ENUMERATOR_ALLOW_PRIMARY)) {
    enumerateWeapons("primary");
  }
  if (yn(process.env.ENUMERATOR_ALLOW_SECONDARY)) {
    enumerateWeapons("secondary");
  }
  if (yn(process.env.ENUMERATOR_ALLOW_MELEE)) {
    enumerateWeapons("melee");
  }
  if (yn(process.env.ENUMERATOR_ALLOW_ARCHWING)) {
    enumerateWeapons("archwing");
  }
  if (yn(process.env.ENUMERATOR_ALLOW_ROBOTIC)) {
