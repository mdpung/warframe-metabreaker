...
function scrapePrimaryStats() {
  scrapeWeaponStats(
    JSON.parse(fs.readFileSync(process.env.PRIMARY_JSON_PRE_STORAGE_PATH)),
    process.env.PRIMARY_HTML_STORAGE_BASE_PATH,
    process.env.PRIMARY_JSON_STATS_STORAGE_BASE_PATH
  );
}

function scrapeSecondaryStats() {
  scrapeWeaponStats(
    JSON.parse(fs.readFileSync(process.env.SECONDARY_JSON_PRE_STORAGE_PATH)),
    process.env.SECONDARY_HTML_STORAGE_BASE_PATH,
    process.env.SECONDARY_JSON_STATS_STORAGE_BASE_PATH
  );
}

function scrapeMeleeStats() {
  scrapeWeaponStats(
    JSON.parse(fs.readFileSync(process.env.MELEE_JSON_PRE_STORAGE_PATH)),
    process.env.MELEE_HTML_STORAGE_BASE_PATH,
    process.env.MELEE_JSON_STATS_STORAGE_BASE_PATH
  );
}

function scrapeArchwingStats() {
  scrapeWeaponStats(
    JSON.parse(fs.readFileSync(process.env.ARCHWING_JSON_PRE_STORAGE_PATH)),
    process.env.ARCHWING_HTML_STORAGE_BASE_PATH,
    process.env.ARCHWING_JSON_STATS_STORAGE_BASE_PATH
  );
}

function scrapeRoboticStats() {
  scrapeWeaponStats(
    JSON.parse(fs.readFileSync(process.env.ROBOTIC_JSON_PRE_STORAGE_PATH)),
    process.env.ROBOTIC_HTML_STORAGE_BASE_PATH,
    process.env.ROBOTIC_JSON_STATS_STORAGE_BASE_PATH
  );
}
...
export function scrapeAllWeaponStats() {
  ...
  if (yn(process.env.SCRAPER_ALLOW_PRIMARY)) {
    console.log("Scraping primaries...");
    scrapePrimaryStats();
  }
  if (yn(process.env.SCRAPER_ALLOW_SECONDARY)) {
    console.log("Scraping secondaries...");
    scrapeSecondaryStats();
  }
  if (yn(process.env.SCRAPER_ALLOW_MELEE)) {
    console.log("Scraping melees...");
    scrapeMeleeStats();
  }
  if (yn(process.env.SCRAPER_ALLOW_ARCHWING)) {
    console.log("Scraping archwings...");
    scrapeArchwingStats();
  }
  if (yn(process.env.SCRAPER_ALLOW_ROBOTIC)) {
